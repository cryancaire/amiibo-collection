  ...
  const loadSharedWishlist = async () => {
    ...
    try {
      ...
      if (shareError || !shareInfo) {
        throw new Error('Shared wishlist not found or no longer available');
      }
      ...
      // Get the wishlist amiibos
      const result = await amiiboService.getUserWishlistWithDetails(shareInfo.user_id);
      if (result.error) {
        ...
        {wishlistAmiibos.length === 0 ? (
          ...
          <div>
            <div style={{
              ...
              {wishlistAmiibos.length} amiibo{wishlistAmiibos.length !== 1 ? 's' : ''} on wishlist
            </div>
